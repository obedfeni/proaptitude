...
export function updateQuestionStats(questionId: string, correct: boolean, category: string = ''): void {
  ...
  if (!data.questionStats[questionId]) {
    data.questionStats[questionId] = { timesAsked: 0, timesCorrect: 0, weight: 1.0, lastAsked: 0, category };
  }
  const stats = data.questionStats[questionId];
  stats.timesAsked++;
  stats.timesCorrect += correct ? 1 : 0;
  ...
  stats.weight = Math.max(0.3, (1.5 - accuracy) * (1 - frequencyFactor * 0.3) + 0.5);
  saveData(data);
}
...
export function getCategoryStatsMap(category: string): Record<string, { timesAsked: number; timesCorrect: number }> {
  const data = loadData();
...
export function getAnalytics() {
  ...
  if (results.length === 0) {
    return { totalTests: 0, averageScore: 0, bestCategory: 'N/A', streakDays: data.streakDays, recentResults: [], categoryPerformance: {} };
  }
  ...
  const bestCategory = Object.entries(categoryPerformance).sort((a, b) => b[1].avg - a[1].avg)[0]?.[0] || 'N/A';
  return { totalTests, averageScore, bestCategory, streakDays: data.streakDays, recentResults: results.slice(-10).reverse(), categoryPerformance };
}
